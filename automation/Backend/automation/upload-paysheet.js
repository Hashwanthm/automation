...
async function processClient(page, client) {
  ...
  // DB SFTP paths can be stored with Windows backslash (\) or legacy slash (/).
  // Support both so old records still upload correctly.
...
async function closeUploadSession(session) {
  await session?.browser?.close().catch(() => {});
}

async function markDirectUploadFailed(client, err) {
...
async function uploadDownloadedClient(session, client) {
  if (!session?.page) {
    throw new Error("Upload session is not ready.");
  }

  await waitWhilePaused();
  if (global.automationStopRequested) return "STOPPED";

  try {
    const result = await processClient(session.page, client);
    await waitWhilePaused();
    return result;
  } catch (err) {
    await markDirectUploadFailed(client, err);
    return "FAILED";
  }
}
...
async function uploadPaysheetBulk(options = {}) {
  ...
  try {
    ...
    for (const client of clients) {
      ...
      const status = await uploadDownloadedClient(session, client);
      results.push({ client: client.client_code, status });
      ...
      await session.page.waitForTimeout(DELAY_BETWEEN_CLIENTS);
    }
  } finally {
    try {
    ...
    await closeUploadSession(session);
...
module.exports.createUploadSession = createUploadSession;
module.exports.closeUploadSession = closeUploadSession;
module.exports.uploadDownloadedClient = uploadDownloadedClient;
module.exports.processClient = processClient;
