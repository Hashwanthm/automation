...
async function runAutomation(data, options = {}) {
    if (!data.length) {
    ...
    for (const client of data) {
        ...
        try {
            ...
            // Optional hook used by the main automation pipeline.
            // It uploads the current client immediately after this client's files
            // are saved instead of waiting for every client download to finish.
            if (typeof options.onClientDownloaded === "function") {
                await options.onClientDownloaded({
                    client_code: client.clientCode,
                    client_name: client.clientName || client.clientCode,
                    entity_code: client.entityCode,
                    month: client.month,
                    sftp_path: client.sftpPath || client.sftp_path || client.filePath || "",
                    hrisFilePath: hrisPath,
                    paysheetFilePath: paysheetPath
                });
            }
