...
async function processClient(page, client) {
  ...
  await runDb(
    `UPDATE automation_runs
     SET upload_status='Processing', run_status='Uploading', updated_at=CURRENT_TIMESTAMP
  ...
  await runDb(
    `UPDATE automation_runs
     SET upload_status='Completed', overall_status='Completed', run_status='Completed', completed_at=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP
     WHERE company_code=? AND entity_code=? AND period=?`,
    [client.client_code, client.entity_code, client.month]
  );
...
async function getClientsToUpload(options = {}) {
  ...
  let sql = `
    ...
    WHERE COALESCE(ar.hris_status, 'Pending') = 'Completed'
      AND COALESCE(ar.paysheet_status, 'Pending') = 'Completed'
      AND COALESCE(ar.upload_status, 'Pending') <> 'Completed'
      AND ar.hris_filepath IS NOT NULL
      AND ar.paysheet_filepath IS NOT NULL
  `;

  if (clientCode) {
    sql += " AND lower(COALESCE(ar.company_code, ''))=lower(?)";
    params.push(clientCode);
  }

  if (entityCode) {
    sql += " AND lower(ar.entity_code)=lower(?)";
  ...
  try {
    ...
    for (const client of clients) {
      ...
      try {
        ...
        await runDb(
          `UPDATE automation_runs
           SET upload_status='Failed', overall_status='Failed', run_status='Failed', error_message=?, updated_at=CURRENT_TIMESTAMP
           WHERE company_code=? AND entity_code=? AND period=?`,
          [err.message, client.client_code, client.entity_code, client.month]
        ).catch((dbErr) => {
