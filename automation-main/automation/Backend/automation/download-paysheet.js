        ...
        try {
            await runDb(
                `UPDATE automation_runs
                 SET hris_status=CASE WHEN COALESCE(hris_status, 'Pending')='Completed' THEN hris_status ELSE 'Processing' END,
                     paysheet_status=CASE WHEN COALESCE(paysheet_status, 'Pending')='Completed' THEN paysheet_status ELSE 'Pending' END,
                     upload_status=CASE WHEN COALESCE(upload_status, 'Pending')='Completed' THEN upload_status ELSE 'Pending' END,
                     run_status='Running',
                     updated_at=CURRENT_TIMESTAMP
                 WHERE company_code=? AND entity_code=? AND period=?`,
            ...
            if (!hrisPath) {
                ...
                await runDb(
                    `UPDATE automation_runs
                     SET hris_filepath=?, hris_status='Completed', run_status='Running', updated_at=CURRENT_TIMESTAMP
                     WHERE company_code=? AND entity_code=? AND period=?`,
            ...
            if (!paysheetPath) {
                ...
                await runDb(
                    `UPDATE automation_runs
                     SET paysheet_filepath=?, paysheet_status='Completed', run_status='Running', updated_at=CURRENT_TIMESTAMP
                     WHERE company_code=? AND entity_code=? AND period=?`,
            ...
            await runDb(
                ` UPDATE automation_runs
                  SET hris_filepath=?, paysheet_filepath=?, hris_status='Completed', paysheet_status='Completed', overall_status='Downloaded', run_status='Downloaded', updated_at=CURRENT_TIMESTAMP
                  WHERE company_code=? AND entity_code=? AND period=?`,
            ...
            await runDb(
                ` UPDATE automation_runs
                  SET hris_filepath=?, paysheet_filepath=?,
                      hris_status=CASE WHEN ? IS NOT NULL AND ? <> '' THEN 'Completed' ELSE 'Failed' END,
                      paysheet_status=CASE WHEN ? IS NOT NULL AND ? <> '' THEN 'Completed' ELSE 'Failed' END,
                      overall_status='Failed', run_status='Failed', error_message=?, updated_at=CURRENT_TIMESTAMP
                  WHERE company_code=? AND entity_code=? AND period=?`,
                [
                    hrisPath || null,
                    paysheetPath || null,
